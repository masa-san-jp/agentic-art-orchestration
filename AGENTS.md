...
Clone/fork users may select the Project-owned repo-local contract with `--project-root <absolute agentic-art-project checkout>` or `AGENTIC_ART_PROJECT_ROOT`. The shared v2 resolver derives all private roots below `.agentic-art/`, runs the pinned Project validator and Git boundary checks before creating anything, and rejects v1 profile/root mixing as `AMBIGUOUS_DESTINATION_MODE`. It never infers a remote, visibility, commit, or publication permission.
