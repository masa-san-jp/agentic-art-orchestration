...
FULL_SELF_DIVERSITY_MIN_ANCHORS = 3
...
def build_self_diversity_report(
    ...
    if len(anchors) == 0:
        ...
        status = "REJECT"
    elif selection_limit >= 10 and len(anchors) >= FULL_SELF_DIVERSITY_MIN_ANCHORS and (distinct_count < 3 or max_share > 0.4):
        status = "REJECT"
    elif len(anchors) < FULL_SELF_DIVERSITY_MIN_ANCHORS:
        status = "PASS_LIMITED_DIVERSITY"
    else:
        status = "PASS"
    self_signals = sorted(
...
def build_selection(
    ...
    if require_self_diversity:
        ...
        if report["status"] == "INSUFFICIENT_SELF_DIVERSITY":
            raise ValueError(_error(source, "INSUFFICIENT_SELF_DIVERSITY", "provide at least one eligible consented self-model anchor"))
    ...
    if require_self_diversity:
        ...
        if report["status"] not in {"PASS", "PASS_LIMITED_DIVERSITY"}:
...
def main() -> int:
    ...
    parser.add_argument("--require-self-diversity", action="store_true", help="require at least one consented self-model anchor; report limited diversity below three")
