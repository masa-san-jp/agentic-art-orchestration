...
（2026-08-25 マサさんの言葉のまま。要約・言い換えをしない。要件は README の「要件」節を正とする。）

以下に述べる基盤・契約・ゲート・qualification は、すべてこの目的のための手段である。手段の完成を目的と読み替えない。本節の以降の記述、および 1.1 の7項目は、この目的をどう実現するかの説明である。
